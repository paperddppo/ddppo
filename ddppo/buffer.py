...
class ReplayMemory:
    ...
    def push(self, state, action, reward, next_state, mask, done):
        ...
        self.buffer[self.position] = tuple([copy.deepcopy(x) for x in [state, action, reward, next_state, mask, done]])
...
class ReplayMemory_without_mask:
    ...
    def push(self, state, action, reward, next_state, done):
        if len(self.buffer) < self.capacity:
            self.buffer.append(None)
        self.buffer[self.position] = tuple([copy.deepcopy(x) for x in [state, action, reward, next_state, done]])
        #print(self.buffer[self.position])
        self.position = (self.position + 1) % self.capacity
        #print(self.position)
